...
// ============================================================================
// CHECKPOINT 8 - FINAL MILESTONE VERSION
// Date: August 28, 2025
// Status: STABLE FOUNDATION FOR FUTURE DEVELOPMENT
// 
// ✅ COMPLETED & OPTIMIZED FEATURES:
// - Frequency indicators: Working from both Templates tab and Routine tab
// - UI improvements: 50px timestamps, multiline text, repositioned kebab menus
// - Performance optimized: All debug statements removed, unused code cleaned
// - Template system: Full CSV parsing with SharedPreferences persistence
// - Timeline interactions: Drag handles, expandable FAB, multi-anchor routing
// - Data integrity: Complete anchor index assignment and frequency tracking
// 
// 🎯 STABLE MILESTONE - USE AS FOUNDATION FOR FUTURE IMPROVEMENTS
// ============================================================================
...
// Static helper function to clear all stored routine data
Future<void> clearAllStoredData() async {
  try {
    ...
    // Silently handle any errors during clearing
  }
}

// Static helper function to clear only routine content, preserving user settings
Future<void> clearRoutineContentOnly() async {
  try {
    final prefs = await SharedPreferences.getInstance();
    ...
    // PRESERVE user settings: dayOffs, scheduleMode, repeatWorkdaysRoutine, stopRoutineOnDayOffs
    // PRESERVE meal times and other user preferences
    // Only clear the routine content (timeline actions)
...
// Helper function to load current user settings
Future<Map<String, dynamic>> loadCurrentUserSettings() async {
  try {
    final prefs = await SharedPreferences.getInstance();
    
    // Load wake and bed times with defaults
    final wakeTime = TimeOfDay(
      hour: prefs.getInt('wakeTime_hour') ?? 6,
      minute: prefs.getInt('wakeTime_minute') ?? 0,
    );
    final bedTime = TimeOfDay(
      hour: prefs.getInt('bedTime_hour') ?? 0,
    ...
    for (int i = 0; i < 3; i++) {
      ...
      if (hour != null && minute != null) {
        ...
        // Default meal times
        switch (i) {
          case 0: mealTimes.add(TimeOfDay(hour: 8, minute: 0)); break;  // Breakfast
          case 1: mealTimes.add(TimeOfDay(hour: 12, minute: 0)); break; // Lunch
          case 2: mealTimes.add(TimeOfDay(hour: 19, minute: 0)); break; // Dinner
        }
      }
      ...
      if (name != null) {
        ...
        // Default meal names
        switch (i) {
          case 0: mealNames.add('Breakfast'); break;
          case 1: mealNames.add('Lunch'); break;
          case 2: mealNames.add('Dinner'); break;
        }
      }
    }
    
    // Load other settings with defaults
    final scheduleMode = prefs.getString('scheduleMode') ?? 'Weekly';
    final dayOffsStrings = prefs.getStringList('dayOffs') ?? ['6', '7'];
    final dayOffs = dayOffsStrings.map((e) => int.parse(e)).toSet();
    final stopRoutineOnDayOffs = prefs.getBool('stopRoutineOnDayOffs') ?? false;
    final repeatWorkdaysRoutine = prefs.getBool('repeatWorkdaysRoutine') ?? true;
    ...
    return {
      ...
      'scheduleMode': 'Weekly',
      'dayOffs': <int>{6, 7},
      'stopRoutineOnDayOffs': false,
      'repeatWorkdaysRoutine': true,
    };
  }
}
    ...
    try {
      ...
      if (mounted) {
        if (isSetupComplete) {
          ...
          final savedBedHour = prefs.getInt('bedTime_hour') ?? 0;
          final savedBedMinute = prefs.getInt('bedTime_minute') ?? 0;
          final savedScheduleMode = prefs.getString('scheduleMode') ?? 'Weekly';
          final isCasualTemplate = prefs.getBool('isCasualTemplate') ?? false;
          
          // Load day-offs and related settings
          final savedDayOffsStrings = prefs.getStringList('dayOffs') ?? ['6', '7'];
          final savedDayOffs = savedDayOffsStrings.map((e) => int.tryParse(e) ?? 0).where((e) => e > 0).toSet();
          final savedStopRoutineOnDayOffs = prefs.getBool('stopRoutineOnDayOffs') ?? false;
          final savedRepeatWorkdaysRoutine = prefs.getBool('repeatWorkdaysRoutine') ?? true;
          
          // Load meal times (with defaults) - dynamically load up to stored meals
          List<TimeOfDay> mealTimes = [];
          List<String> mealNames = [];
          for (int i = 0; i < 10; i++) { // Check up to 10 meals
            final hour = prefs.getInt('meal${i}_hour');
            final minute = prefs.getInt('meal${i}_minute');
            final name = prefs.getString('meal${i}_name');
            ...
            if (hour != null && minute != null) {
              ...
              // Add defaults for first 3 meals if not found
              switch (i) {
                case 0: 
                  mealTimes.add(TimeOfDay(hour: 8, minute: 0));
                  mealNames.add('Breakfast');
                  break;
                case 1: 
                  mealTimes.add(TimeOfDay(hour: 12, minute: 0));
                  mealNames.add('Lunch');
                  break;
                case 2: 
                  mealTimes.add(TimeOfDay(hour: 19, minute: 0));
                  mealNames.add('Dinner');
                  break;
              }
            } else {
              break; // No more meals found
            }
          }
          ...
          Navigator.of(context).pushReplacement(
            MaterialPageRoute(
              builder: (context) => HomeScreen(
                ...
                dayOffs: savedDayOffs.isNotEmpty ? savedDayOffs : {6, 7}, // Default Sat & Sun if no saved data
...
class _ManualSetupScreenState extends State<ManualSetupScreen> {
  ...
  Set<int> dayOffs = {6, 7}; // 1=Mon, 2=Tue, ..., 7=Sun - Default Sat & Sun
  ...
  void _loadCurrentSettings() async {
    final settings = await loadCurrentUserSettings();
    setState(() {
      wakeTime = settings['wakeTime'];
      bedTime = settings['bedTime'];
      mealTimes = List<TimeOfDay>.from(settings['mealTimes']);
      mealNames = List<String>.from(settings['mealNames']);
      scheduleMode = settings['scheduleMode'];
      dayOffs = Set<int>.from(settings['dayOffs']);
      stopRoutineOnDayOffs = settings['stopRoutineOnDayOffs'];
      repeatWorkdaysRoutine = settings['repeatWorkdaysRoutine'];
    });
  }
                ...
                children: [
                    ...
                    children: List.generate(7, (i) {
                      ...
                      return SizedBox(
                        ...
                        child: FilterChip(
                          ...
                          label: Text(_weekdayName(day), style: TextStyle(fontSize: 12)),
                          selected: isOff,
                  ...
                  SwitchListTile(
                    ...
                    onChanged: (value) async {
                      ...
                      if (mounted) {
                        setState(() {
                          // The settings change will be picked up by didUpdateWidget
                          // which will properly refresh only template actions while preserving events
                        });
                      }
                    },
                  ),
...
class _ActionPickerScreenState extends State<ActionPickerScreen> {
    ...
    try {
      ...
      setState(() {
        allActions = jsonData.map((item) {
          List<String> recommendedTimes = List<String>.from(item['recommendedTimes'] ?? []);
          List<String> recommendedDays = List<String>.from(item['recommendedDays'] ?? []);
          
          // Set timeOfDay based on primary recommended time
          String timeOfDay = 'All Day';
          if (recommendedTimes.isNotEmpty) {
            timeOfDay = recommendedTimes.first; // Use first recommended time as primary
          }
          
          // Set recommended time based on time period
          TimeOfDay time = _getRecommendedTimeForPeriod(timeOfDay);
          
          // Set dayOfWeek based on recommended days
          String dayOfWeek = 'Daily';
          if (recommendedDays.isNotEmpty) {
            if (recommendedDays.contains('Weekdays') && recommendedDays.contains('Day-offs')) {
              dayOfWeek = 'Daily';
            } else if (recommendedDays.contains('Weekdays')) {
              dayOfWeek = 'Weekdays';
            } else if (recommendedDays.contains('Day-offs')) {
              dayOfWeek = 'Day-offs';
            }
          }
  ...
  TimeOfDay _getRecommendedTimeForPeriod(String timeOfDay) {
    switch (timeOfDay) {
      case 'Morning':
      ...
      default: // 'All Day' or any other value
        return TimeOfDay(hour: 10, minute: 0); // Mid-morning as default
    }
  }
  ...
  List<Map<String, dynamic>> get filteredAndSortedActions {
    ...
    try {
      if (sortBy == 'actions') {
        ...
        // Improved sorting logic for better time period prioritization
        Map<String, int> timeOrder = {};
        if (timeSortOrder == 'morning') {
          // Morning prioritizes: Morning actions first, then actions that include Morning, then chronological
          timeOrder = {'Morning': 1, 'Noon': 2, 'Afternoon': 3, 'Evening': 4, 'Night': 5, 'All Day': 6};
        } else if (timeSortOrder == 'afternoon') {
          timeOrder = {'Afternoon': 1, 'Noon': 2, 'Evening': 3, 'Night': 4, 'Morning': 5, 'All Day': 6};
        } else if (timeSortOrder == 'evening') {
          timeOrder = {'Evening': 1, 'Night': 2, 'Afternoon': 3, 'Noon': 4, 'Morning': 5, 'All Day': 6};
        } else { // 'all' - natural chronological order
          timeOrder = {'Morning': 1, 'Noon': 2, 'Afternoon': 3, 'Evening': 4, 'Night': 5, 'All Day': 6};
        }
        
        filtered.sort((a, b) {
          // Primary sort by time period priority
          int timeA = timeOrder[a['timeOfDay']] ?? 7;
          int timeB = timeOrder[b['timeOfDay']] ?? 7;
          if (timeA != timeB) return timeA.compareTo(timeB);
          
          // Secondary sort: for actions with multiple recommendedTimes, prioritize based on sort mode
          if (timeSortOrder == 'morning') {
            List<String> recTimesA = List<String>.from(a['recommendedTimes'] ?? []);
            List<String> recTimesB = List<String>.from(b['recommendedTimes'] ?? []);
            
            // If both include Morning, sort by actual time
            if (recTimesA.contains('Morning') && recTimesB.contains('Morning')) {
            ...
            // Prioritize actions that include Morning
            if (recTimesA.contains('Morning') && !recTimesB.contains('Morning')) return -1;
            if (!recTimesA.contains('Morning') && recTimesB.contains('Morning')) return 1;
          }
          
          // Tertiary sort by actual time
          TimeOfDay timeOfDayA = a['time'] as TimeOfDay;
          TimeOfDay timeOfDayB = b['time'] as TimeOfDay;
          return (timeOfDayA.hour * 60 + timeOfDayA.minute).compareTo(timeOfDayB.hour * 60 + timeOfDayB.minute);
        });
                          ...
                          Expanded(
                            child: Column(
                              ...
                              children: [
                                ...
                                // Show description for events
                                if (action['category'] == 'event' && action['description'] != null && action['description'].toString().isNotEmpty)
                                  Padding(
                                ...
                                Row(
                                  children: [
                                    ...
                                    SizedBox(width: 4),
                                    // Special styling for event category
                                    (action['category']?.toString().toLowerCase() == 'event') 
                                      ? Text(
                                          'Event', 
                                          style: TextStyle(
                                            color: Color(0xFFDC143C), // Scarlet red
                                            fontSize: 12,
                                            fontWeight: FontWeight.w600
                                          )
                                        )
                                      : Text(
                                          '${action['category']?.toString().split(' ').map((word) => word.isNotEmpty ? word[0].toUpperCase() + word.substring(1).toLowerCase() : word).join(' ')}',
...
class _HomeScreenState extends State<HomeScreen> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        ...
        children: [
          ...
          SettingsTab(
            ...
            onWakeTimeChanged: (time) async {
              ...
              final prefs = await SharedPreferences.getInstance();
              await prefs.setInt('wakeTime_hour', time.hour);
              await prefs.setInt('wakeTime_minute', time.minute);
            ...
            onBedTimeChanged: (time) async {
              setState(() => bedTime = time);
              
              // Save to SharedPreferences
              final prefs = await SharedPreferences.getInstance();
              await prefs.setInt('bedTime_hour', time.hour);
              await prefs.setInt('bedTime_minute', time.minute);
            ...
            onMealTimesChanged: (times) async {
              setState(() => mealTimes = times);
              
              // Save to SharedPreferences
              final prefs = await SharedPreferences.getInstance();
              for (int i = 0; i < times.length; i++) {
                await prefs.setInt('meal${i}_hour', times[i].hour);
                await prefs.setInt('meal${i}_minute', times[i].minute);
              }
            ...
            onMealNamesChanged: (names) async {
              setState(() => mealNames = names);
              
              // Save to SharedPreferences
              final prefs = await SharedPreferences.getInstance();
              for (int i = 0; i < names.length; i++) {
                await prefs.setString('meal${i}_name', names[i]);
              }
            },
            onRepeatWorkdaysRoutineChanged: (value) async {
              setState(() {
              ...
              // Save to SharedPreferences
              final prefs = await SharedPreferences.getInstance();
              await prefs.setBool('repeatWorkdaysRoutine', value);
            ...
            onScheduleModeChanged: (mode) async {
              setState(() => scheduleMode = mode);
              
              // Save to SharedPreferences
              final prefs = await SharedPreferences.getInstance();
              await prefs.setString('scheduleMode', mode);
            },
            onSettingsChanged: () {
      ...
      floatingActionButton: currentIndex == 1 ? ExpandableFAB(
        routineTabKey: routineTabKey,
      ) : null,
      ...
      children: [
        Card(
          child: ListTile(
            ...
            onTap: () async {
              // Clear routine content only, preserving user settings
              await clearRoutineContentOnly();
              Navigator.push(context, MaterialPageRoute(builder: (_) => ManualSetupScreen()));
            },
          ),
        ),
        SizedBox(height: 16),
...
class _RoutineTabState extends State<RoutineTab> {
  ...
  // SCROLL CONTROLLER: For auto-scrolling to current action
  ...
  void initState() {
    ...
    // Update current time every minute and check for current action changes
    Timer.periodic(Duration(minutes: 1), (timer) {
      if (mounted) {
        setState(() {
          currentTime = DateTime.now();
          _updateCurrentActiveAction();
        });
      }
    });
  }
  ...
  void _addCustomEvent(Map<String, dynamic> eventData) {
    ...
    // Add to day-specific actions
    if (!daySpecificActions.containsKey(dayKey)) {
      daySpecificActions[dayKey] = [];
    }
    daySpecificActions[dayKey]!.add(eventAction);
    
    // If this is today's date, update display immediately
    DateTime currentDate = selectedDate;
    String currentDayKey = '${currentDate.year}-${currentDate.month.toString().padLeft(2, '0')}-${currentDate.day.toString().padLeft(2, '0')}';
    
    if (dayKey == currentDayKey) {
      setState(() {
        displayActions.add(eventAction);
        
        // Re-sort all actions by time
        displayActions.sort((a, b) {
          final timeA = a['time'] as TimeOfDay;
          final timeB = b['time'] as TimeOfDay;
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text('Event "$name" created for ${date.day}/${date.month}/${date.year}'),
  ...
  Future<void> _initializeDisplayActions() async {
    ...
    if (widget.isCasualTemplate) {
      ...
      // Load template logic:
      // 1) No stored routine actions exist -> load templates (events don't count)
      // 2) Template explicitly requested from template tab -> load templates (override stored actions)
      // 3) Stored routine actions exist -> use them regardless of repeat weekdays setting
      bool hasStoredActions = daySpecificActions.containsKey(dayKey);
      bool hasNonEventActions = false;
      if (hasStoredActions) {
        // Check if there are any non-event actions
        hasNonEventActions = daySpecificActions[dayKey]!.any((action) => 
          action['isCustomEvent'] != true
        );
      }
      bool shouldLoadTemplate = !hasNonEventActions; // Only load if no routine actions exist (events are OK)
      ...
      if (shouldLoadTemplate) {
        try {
          ...
          // Preserve existing events before storing template actions
          List<Map<String, dynamic>> existingEvents = [];
          if (hasStoredActions && daySpecificActions.containsKey(dayKey)) {
            existingEvents = daySpecificActions[dayKey]!
                .where((action) => action['isCustomEvent'] == true)
                .toList();
          }
          
          // Store template actions
          daySpecificActions[dayKey] = dayActions;
          
          // Merge template actions with existing events
          if (existingEvents.isNotEmpty) {
            daySpecificActions[dayKey]!.addAll(existingEvents);
          }
          ...
          // Preserve existing events before storing fallback template actions
          List<Map<String, dynamic>> existingEvents = [];
          if (hasStoredActions && daySpecificActions.containsKey(dayKey)) {
            existingEvents = daySpecificActions[dayKey]!
                .where((action) => action['isCustomEvent'] == true)
    ...
    if (widget.stopRoutineOnDayOffs) {
      ...
      if (widget.dayOffs.contains(currentWeekday)) {
        ...
        displayActions.clear();
        // Optionally, you could keep schedule times like wake/sleep if needed:
        // displayActions.removeWhere((action) => action['isScheduleTime'] != true);
      }
    }
  ...
  // Helper function to check if the current selected date is a day-off with the setting enabled
  bool _isDayOff() {
    if (!widget.stopRoutineOnDayOffs) return false; // Setting is OFF
    final currentWeekday = selectedDate.weekday; // 1=Monday, 7=Sunday
    return widget.dayOffs.contains(currentWeekday);
  }
  ...
  // CURRENT ACTION DETECTION AND AUTO-SCROLL FUNCTIONALITY
  void _updateCurrentActiveAction() {
    if (!_isSameDay(selectedDate, DateTime.now())) {
      // Not viewing today, no need to highlight current action
    ...
    // Find the nearest anchor that just passed (most recent past action)
    for (int i = displayActions.length - 1; i >= 0; i--) {
      final action = displayActions[i];
      final actionTime = action['time'] as TimeOfDay;
      final actionMinutes = actionTime.hour * 60 + actionTime.minute;
      
      // Handle next-day logic (if sleep time is past midnight)
      bool isActionPast = false;
      final wakeMinutes = (widget.wakeTime ?? TimeOfDay(hour: 6, minute: 0)).hour * 60 + (widget.wakeTime ?? TimeOfDay(hour: 6, minute: 0)).minute;
      final bedMinutes = (widget.bedTime ?? TimeOfDay(hour: 0, minute: 0)).hour * 60 + (widget.bedTime ?? TimeOfDay(hour: 0, minute: 0)).minute;
      ...
      if (bedMinutes < wakeMinutes) {
        ...
        if (actionMinutes < wakeMinutes) {
          // Action is in "next day" period, skip for today's current action detection
          continue;
        }
      }
      
      isActionPast = actionMinutes <= currentMinutes;
  ...
  void _scrollToAction(int actionIndex) {
    ...
    // Calculate scroll position (each card is approximately 84px height + 12px margin)
  ...
  bool _isCurrentActiveAction(int index) {
    if (!_isSameDay(selectedDate, DateTime.now())) {
      return false; // No highlighting if not today
    }
    
    final now = DateTime.now();
    if (index >= displayActions.length) return false;
    
    final currentAction = displayActions[index];
    final currentActionTime = currentAction['time'] as TimeOfDay;
    
    // Check if this action's time exactly matches current time (within same minute)
    if (currentActionTime.hour == now.hour && currentActionTime.minute == now.minute) {
      return true;
    }
    
    // Find the next action that hasn't happened yet
    int? nextActionIndex;
    
    for (int i = 0; i < displayActions.length; i++) {
      final actionTime = displayActions[i]['time'] as TimeOfDay;
      final actionDateTime = DateTime(now.year, now.month, now.day, actionTime.hour, actionTime.minute);
      
      if (actionDateTime.isAfter(now)) {
        nextActionIndex = i;
        break;
      }
    }
    
    // Highlight if this is the next action
    return nextActionIndex == index;
  }
              ...
              PopupMenuButton<String>(
                ...
                onSelected: (value) async {
                  ...
                  if (value == 'clear_day') {
                    ...
                    final selectedTemplate = await _showTemplateSelectionDialog(context);
                    
                    if (selectedTemplate != null) {
                      // Show confirmation dialog for the selected template
                      final shouldApply = await showDialog<bool>(
                        context: context,
                        builder: (BuildContext context) {
                          return AlertDialog(
                            title: Text('Apply Template'),
                            content: Text('Apply "$selectedTemplate" template to this day? This will replace any existing actions.'),
                            actions: [
                              TextButton(
                                onPressed: () => Navigator.of(context).pop(false),
                                child: Text('Cancel'),
                      ...
                      if (shouldApply == true) {
                        await _applySelectedTemplate(selectedTemplate, dayKey);
                      }
                    }
                  }
                },
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    Text(
                      ...
                      style: TextStyle(
                        fontSize: 18, 
                        color: _isDayOff() ? Colors.green : Colors.grey,
                        fontWeight: _isDayOff() ? FontWeight.w500 : FontWeight.normal,
                      )
                    ),
                    if (!_isDayOff()) ...[
                      SizedBox(height: 8),
  ...
  Widget _buildVerticalTimeline() {
    // Update current active action when building timeline
    WidgetsBinding.instance.addPostFrameCallback((_) {
      _updateCurrentActiveAction();
    });
    
    return ReorderableListView.builder(
      scrollController: _scrollController, // Use scrollController parameter
      ...
      itemBuilder: (context, index) {
        ...
        switch (category) {
          ...
          case 'schedule':
            ...
            break;
          case 'event':
            categoryColor = Color(0xFFDC143C); // Scarlet red for events
            displayCategory = 'Event';
            break;
          default:
            categoryColor = Colors.grey;
            displayCategory = category.split(' ').map((word) => word.isNotEmpty ? word[0].toUpperCase() + word.substring(1).toLowerCase() : word).join(' ');
        }
        ...
        // Check if this is the current active action for highlighting
        final isCurrentAction = _isCurrentActiveAction(index);
                  ...
                  children: [
                    Text(
                      ...
                      style: TextStyle(
                        fontSize: 16,
                        fontWeight: FontWeight.bold,
                        color: isCurrentAction ? Colors.red : Theme.of(context).primaryColor,
                      ),
                    ),
                    Text(
                      ...
                      style: TextStyle(
                        fontSize: 12,
                        color: isCurrentAction ? Colors.red.withOpacity(0.7) : Colors.grey[600],
                      ),
                    ),
                  ],
                ),
                    ...
                    decoration: BoxDecoration(
                      ...
                      border: Border.all(
                        color: isCurrentAction
                          ? categoryColor.withOpacity(0.8)  // Enhanced category color for current/next action
                          : (_isActionDisabled(action)
                            ? Colors.grey[300]!.withOpacity(0.3) // Faded border for disabled
                            : categoryColor.withOpacity(0.2)),
                        width: isCurrentAction ? 1.5 : 1.5,  // Same width for all actions
                      ),
                                ...
                                children: [
                                  ...
                                  Row(
                                    children: [
                                      ...
                                      // Frequency indicator for template actions - show original frequency
                                      if (action.containsKey('frequency') && 
                                          (action['frequency'] ?? 1) > 1 &&
                                      ...
                                      if (action.containsKey('anchorIndex') && 
                                          !(action.containsKey('frequency') && (action['frequency'] ?? 1) > 1) &&
                                          !(actionName.toLowerCase().contains('wake') || actionName.toLowerCase().contains('sleep') || actionName.contains('🌅') || actionName.contains('😴'))) ...[
                                        Container(
                                          padding: EdgeInsets.symmetric(horizontal: 8, vertical: 4),
                                          decoration: BoxDecoration(
                                  ...
                                  if (action['category']?.toString().toLowerCase() == 'event' && 
                                      action['description'] != null && 
                                      action['description'].toString().isNotEmpty) ...[
                                    SizedBox(height: 4),
                                    Text(
                                      action['description'],
                                      style: TextStyle(
  ...
  void _duplicateAnchor(int index) {
    ...
    // Split anchors: get anchors from the duplicated anchor onward for redistribution
  ...
  Future<String?> _showTemplateSelectionDialog(BuildContext context) async {
              ...
              children: [
                Card(
                  child: ListTile(
                    ...
                    subtitle: Text('Balanced daily routine'),
                    trailing: Text('FREE', style: TextStyle(color: Colors.green, fontWeight: FontWeight.bold)),
                    onTap: () => Navigator.of(context).pop('The Casual'),
                  ),
                ),
                Card(
                  child: ListTile(
                    ...
                    onTap: () {
                      ScaffoldMessenger.of(context).showSnackBar(
                        SnackBar(content: Text('This template is coming soon!')),
                      );
                    },
                  ),
                ),
  ...
  // Apply selected template to a specific day
  Future<void> _applySelectedTemplate(String templateName, String dayKey) async {
    if (templateName == 'The Casual') {
      try {
        // Load The Casual template from CSV
        final (settings, actions) = await CasualTemplateParser.parseFromAsset('assets/data/The_Casual_Template.csv');
        ...
        final selectedDateForTemplate = DateTime(year, month, day);
        final weekday = selectedDateForTemplate.weekday; // 1=Monday, 7=Sunday
        
        // Filter actions for this day and spread anchors
        final expandedActions = actions
            .where((action) => action.dayOfWeek == weekday)
            .expand((action) => action.spreadAnchors(settings.sleepTime))
            .toList();
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('$templateName template applied successfully!'),
            backgroundColor: Colors.green,
          ),
        );
      } catch (e) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('Error applying template. Please try again.'),
            backgroundColor: Colors.red,
          ),
        );
      }
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('$templateName template is coming soon!'),
        ),
      );
    }
  }
            ...
            RichText(
              text: TextSpan(
                ...
                children: [
                  TextSpan(text: '$time • '),
                  TextSpan(
                    text: category.toLowerCase() == 'event' ? 'Event' : category.split(' ').map((word) => word.isNotEmpty ? word[0].toUpperCase() + word.substring(1).toLowerCase() : word).join(' '),
                    style: TextStyle(
                      color: category.toLowerCase() == 'event' ? Color(0xFFDC143C) : null,
                      fontWeight: category.toLowerCase() == 'event' ? FontWeight.w600 : null,
                    ),
                  ),
                ],
              ),
...
// Expandable FAB Widget with Add Action and Create Event options
class ExpandableFAB extends StatefulWidget {
  final GlobalKey<_RoutineTabState> routineTabKey;
  
  const ExpandableFAB({Key? key, required this.routineTabKey}) : super(key: key);
...
class _ExpandableFABState extends State<ExpandableFAB> with TickerProviderStateMixin {
  ...
  void _showCreateEventDialog() async {
    ...
    if (result != null) {
      ...
      final routineState = widget.routineTabKey.currentState;
      if (routineState != null) {
        routineState._addCustomEvent(result);
      }
    }
  }
  ...
  Widget build(BuildContext context) {
    return Stack(
      children: [
        // Option bubbles positioned relative to FAB
        AnimatedBuilder(
          animation: _animation,
          builder: (context, child) {
            return Stack(
              children: [
                // Add Action option (positioned above FAB)
                Positioned(
                  bottom: 16 + 56 + 8 + (8 * _animation.value), // FAB bottom + FAB height + small gap + minimal offset
                  right: 16,
                  child: Transform.scale(
                ...
                // Create Event option (positioned to the left of FAB)
                Positioned(
                  bottom: 16 + 4, // FAB bottom + small offset
                  right: 16 + 56 + 8 + (8 * _animation.value), // FAB right + FAB width + small gap + minimal offset
                  child: Transform.scale(
                    scale: _animation.value,
                    child: Opacity(
                      opacity: _animation.value,
        ...
        // Main FAB - positioned last to be on top
        Positioned(
          bottom: 16,
          right: 16,
          child: FloatingActionButton(
            ...
            child: AnimatedRotation(
              turns: _isExpanded ? 0.125 : 0.0, // 45 degree rotation when expanded
              duration: Duration(milliseconds: 300),
